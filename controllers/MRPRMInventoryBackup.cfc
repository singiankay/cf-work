...
{	
	...
	function getWip() {
		...
		for(name in materialNames) {
			...
			for(m in models) {
				for(bom in boms) {
					if(bom.material_id == name.ItemCode && bom.model_no == m) {
						var process_order = bom.process_order;
						var mat_qty = bom.qty;
					}
				}
				for(bom2 in boms) {
							...
							if(wip.model_id == m) {
								for(wipline in wip.wiplines) {
									if(wipline.process_id == bom2.process) {
										if(wipline.qty > 0) {
											if(bom2.process_order >= process_order) {
												qty += (mat_qty * wipline.qty);
												// arrayAppend(qty, {
												// 	model: m,
												// 	process_id: wipline.process_id,
												// 	bom2_process_id: bom2.process,
												// 	process_order: bom2.process_order,
												// 	bom2_qty: mat_qty,
												// 	wipline_qty: wipline.qty
												// });
											}
										}
									}
		...
		// 			for(wip in wips) {
		// 				if(bom.model_no == wip.model_id) {
		// 					for(wipline in wip.wiplines) {
		// 						if(bom.process == wipline.process_id) {
		// 							arrayAppend(result, {
		// 								material_no: name.ItemCode,
		// 								material_name: name.ItemName,
		// 								material_group: name.ItmsGrpCod,
	...
	function getMaterialNames(materialIDs, division) {
		var mg = model("materialgroupings").findAll(select="code", where="division = '#params.division#' AND type = 'Materials'");

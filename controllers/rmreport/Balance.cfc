component displayname="RM Report - Balance" extends="app.controllers.Controller"
{
	title = "RM Report - Balance";
	...
	function index()
	{
		if(structKeyExists(params, "qrcode")) {
			redirectTo(route="rmreportbalance", key=listlast(trim(params.qrcode),'|'));
		}
		else {
			param name="params.search.mat_no" default="";
			param name="params.search.mat_name" default="";
			param name="params.search.supplier_lot" default="";
			param name="params.search.npi_lot" default="";
			param name="params.search.supplementary_lot" default="";
			param name="params.search.docentry" default="";
			param name="params.search.id_record" default="";
			param name="params.search.view" default="balance";
			...
			search = model("rmreportbalance").new(argumentCollection=params.search);
			isSearch = search.getPartial();
			isTotal = search.getTotal();
	...
	function show()
	{
		controllerJS = "rmreport/rmbalance_show";
		rcv = model("orcv").findAll(select="", where="U_IDrecord = #params.key# AND Canceled = 'N'");
		if(rcv.recordCount == 1) {
			itf = model("oitf").findAll(select="", where="U_BaseRef = #rcv.Docentry# AND Canceled = 'N'", order="DocEntry ASC");
			iss = model("oiss").findAll(select="", where="U_BaseRef = #rcv.Docentry# AND Canceled = 'N'", order="DocEntry ASC");
			rmstatus = getRMStatus(params.key);
			locations = model("owhs").findAll(select="U_IdRecord, WhsCode");
		}
	}
	...
	function generate()
	{
		...
		if(!isAdmin()) {
			flashInsert(error="You are not allowed to generate this file");
		}
		else {
			search = model("rmreportbalance").new(argumentCollection=params.search);
			isSearch = search.getFull();

			if (!isSearch) {
				flashInsert(error="There was an error with your search filters");
			}
			else {
				var spreadsheetObj = spreadsheetNew('RM Inventory Balance');
				...
				var headerList = "Division,Area,Material No,Material Name,Classification,Material type,Date Received,Expiration Date,Remarks,QR Code,Supplier Lot,NPI Lot,Supplementary Lot,Qty In,Qty Out,Qty Balance,Qty NCP,Qty Reinspect,Qty On Hold,Qty for IQC,IQC-G,IQC-Q,IQC-S,WHS-G,WHS-R,WHS-S,PCK-G,Docentry,ID record";
	...
	function getLocation(locations, location)
	{
		for(var l in arguments.locations) {
			if(l.U_Idrecord == arguments.location) {
				return l.WhsCode;
			}
		}
		return "";
	...
	private function getRMStatus(id)
	{
		...
		sqlQuery.setSQL("
			SELECT TOP 1 rcv.U_ProductLine AS division, itm.U_ProductionLine AS area, rcv.DocEntry AS docentry, rcv.U_IDRecord AS id, U_ItemType AS item_type, itm.ItmsGrpCod AS subgroup_code, itb.ItmsGrpNam AS subgroup_name, rcv.U_ItemCode AS material_no, rcv.U_ItemDescription AS material_name, rcv.U_Reference AS form_number, rcv.U_Fifo AS receive_date, rcv.U_ExpiryDate AS expiration_date, rcv.U_InvNo AS invoice_no, rcv.U_PONo AS po_no, rcv.U_QRcode AS qr_code, rcv.U_LotCode AS supplier_lot, rcv.U_LotCode2 AS npi_lot, rcv.U_LotCode3 AS supplementary_lot, 
			 ...
			 ORDER BY rcv.U_ProductLine, rcv.U_ItemCode, rcv.DocEntry DESC
		");
		sqlQuery.addParam(name="id", value=arguments.id, CFSQLTYPE="CF_SQL_INTEGER");
		return sqlQuery.execute().getResult();
	}

	private function getExpirationRemarks(expiration_date)
	{
	...
	private function getExpirationColor(expiration_date)
	{
		if(isDate(arguments.expiration_date)) {
			var d = dateDiff('d', now(), arguments.expiration_date);
			if(d > 14) {

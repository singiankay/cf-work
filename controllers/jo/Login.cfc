component displayname="JO Login" extends="app.controllers.Controller"
{
	...
	function new() 
	{
		if(structKeyExists(SESSION, "jo.username")) {
	...
	function create()
	{
		...
		if(isObject(login)) {
			var loginErp = model("user").findAll(select="fk_user_id, division, role, access_type", where="fk_user_id = #login.hris_id# AND access_type = 'JO' AND is_active = 1", returnAs="query");
			...
			if(loginErp.recordCount) {
				...
				lock scope="Session" timeout="2" type="exclusive" {
					SESSION.jo.user_id = login.hris_id;
					SESSION.jo.username = login.username;
					SESSION.jo.lastname =  employee.lastname;
					SESSION.jo.firstname =  employee.firstname;
					SESSION.jo.middlename =  employee.middlename;
					SESSION.jo.position =  employee.position;
					SESSION.jo.datehired =  employee.date_hired;
					SESSION.jo.allowed_roles = allowedRoles;
					SESSION.jo.allowed_divisions = allowedDivisions;
					SESSION.jo.active_division = params.division;
					
					if(employee.image_path == "") {
						SESSION.jo.picture = "http://npi-appserver/employee/econtacts/img/void.png";
					}
					else {
						SESSION.jo.picture = "http://npi-appserver/employee/personnel/"&employee.image_path;
					}
				}
	...
	function getUser() 
	{
		try {
			...
			if(user.recordCount) {
				var login = model("user").findAll(select="id, access_type, area, division, is_active, role", where = "fk_user_id = '#user.hris_id#' AND access_type = 'JO'");
				var divisions = super.getDivisions();
